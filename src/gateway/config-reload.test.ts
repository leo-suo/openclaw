...
describe("startGatewayConfigReloader", () => {
  ...
  it("queues restart when an external plugin source write only changes the managed index", async () => {
    ...
    expect(harness.onRestart).toHaveBeenCalledWith(
      expect.objectContaining({
        changedPaths: ["installedPluginIndex.installRecords.lossless-claw"],
        restartGateway: true,
        restartReasons: ["installedPluginIndex.installRecords.lossless-claw"],
      }),
      activeConfig,
    );
  ...
  it("queues restart when an external plugin source write also changes plugin config", async () => {
    ...
    expect(harness.onRestart).toHaveBeenCalledWith(
      expect.objectContaining({
        changedPaths: ["plugins.entries", "installedPluginIndex.installRecords.lossless-claw"],
        restartGateway: true,
        restartReasons: ["installedPluginIndex.installRecords.lossless-claw"],
      }),
      nextConfig,
