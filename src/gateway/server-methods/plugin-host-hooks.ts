...
const SESSION_ACTION_SUCCESS_RESULT_KEYS = new Set(["ok", "result", "reply", "continueAgent"]);
...
export const pluginHostHookHandlers: GatewayRequestHandlers = {
  ...
  "plugins.sessionAction": async ({ params, client, respond }) => {
    ...
    try {
      ...
      if (success?.result !== undefined && !isPluginJsonValue(success.result)) {
        respond(
          false,
          undefined,
          errorShape(
      ...
      respond(true, {
        ok: true,
        ...(success?.result !== undefined ? { result: success.result } : {}),
